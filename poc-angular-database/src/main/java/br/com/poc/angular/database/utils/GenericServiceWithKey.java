...
public abstract class GenericServiceWithKey<T extends BaseBean, K extends BaseEntity, IDB extends Serializable, IDE extends Serializable> implements IServiceBase<T, IDB> {
	...
	protected abstract JpaRepository<K, IDE> getConcreteDao();
	...
	public T findById(IDB id) {
		return parserEntity(this.getConcreteDao().findOne(parserKeyBean(id)));
	}
	...
	public T insert(T bean) {
		K idb = getConcreteDao().saveAndFlush(parserBean(bean));
		return parserEntity(idb);
	}
	...
	@Override
	public void delete(IDB bean) {
		this.getConcreteDao().delete(parserKeyBean(bean));
	}
